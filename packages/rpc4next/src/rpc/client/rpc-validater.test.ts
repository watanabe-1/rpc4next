...
describe("createRpcClient", () => {
  ...
  describe("createRpcClient basic behavior", () => {
    it("should send correct JSON body and receive expected response", async () => {
      server.use(
        http.post("http://localhost:3000/api/none/json", async ({ request }) => {
  ...
  it("should validate headers correctly", async () => {
    server.use(
      http.post("http://localhost:3000/api/none/headers", async ({ request }) => {
        const headers = {
  ...
  it("should validate cookies correctly", async () => {
    server.use(
      http.post("http://localhost:3000/api/none/cookies", async ({ request }) => {
  ...
  it("should validate optionalQuery correctly", async () => {
    server.use(
      http.post("http://localhost:3000/api/none/optionalQuery", async ({ request }) => {
        const url = new URL(request.url);
        const query = searchParamsToObject(url.searchParams);
...
describe("createHandler type definitions", () => {
  it("should infer types correctly", async () => {
    ...
    expectTypeOf<Parameters<typeof _post1>>().toEqualTypeOf<ExpectedJsonParameters>();
    ...
    type ExpectedAllParameters = [
      ...
      option?: RpcClientOptions<"Cookie" | "Content-Type", "headers" | "headersInit" | "body">,
    ];
